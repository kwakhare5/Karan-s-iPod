...
export const useMusicPlayer = () => {
    ...
    (event: YTPlayerEvent) => {
      ...
      switch (event.data) {
        ...
        case YT_STATE.PAUSED:
          setState((prev: MusicPlayerState) => ({ ...prev, isPlaying: false, isLoading: false }));
          break;
        case YT_STATE.ENDED:
          handleTrackEnd();
          break;
  ...
  const playSong = useCallback(
    (track: Track, newQueue?: Track[]) => {
      ...
      if (newQueue) {
        setState((prev: MusicPlayerState) => {
          if (prev.isShuffled) {
            const others = [...newQueue].filter((t: Track) => t.videoId !== track.videoId);
            // basic shuffle
        ...
        setState((prev: MusicPlayerState) => {
          const foundIndex = prev.queue.findIndex((t: Track) => t.videoId === track.videoId);
          if (foundIndex !== -1) {
      ...
      setTimeout(() => {
        ...
        let idx = q.findIndex((t: Track) => t.videoId === track.videoId);
        if (idx === -1) idx = 0;
        playFromQueue(q, idx);
      }, 0);
    },
  ...
  const setVolume = useCallback((vol: number) => {
    ...
    setState((prev: MusicPlayerState) => ({ ...prev, volume: clamped }));
  }, []);
  ...
  const toggleShuffle = useCallback(() => {
    setState((prev: MusicPlayerState) => {
      if (!prev.isShuffled) {
        ...
        const others = prev.queue.filter((t: Track) => t.videoId !== prev.currentTrack?.videoId);
        for (let i = others.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [others[i], others[j]] = [others[j], others[i]];
        }
